Device Driver Kata Requirements Specification
=============================================
...
- Then read the value in address 0x0 and check bit 1 in the returned data, also known as the 'ready bit'. (Bit number 1 is the bit left of the least significant bit.) Repeat the read operation until the ready bit is set to 1. This means the write operation is complete. In a typical device it should take around ten microseconds, but it will vary from write to write.
...
- bit 3: protected block error. You cannot write to that address, but other addresses may work.
- bit 4: internal error. Something went wrong this time but next time it might work.
- bit 5: Vpp error. The voltage on the device was wrong and it is now physically damaged.
